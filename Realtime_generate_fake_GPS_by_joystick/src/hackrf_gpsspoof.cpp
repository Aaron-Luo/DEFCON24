...
#pragma comment(lib,"hackrf_static.lib")
 ...
 */
int readRinexNav(ephem_t eph[],double currenttime, const char *fname)
{
	FILE *fp;
	int nsat;
	int sv;
	...
	while (1)
	{
		...
		tmp[2] = 0;
		sv = atoi(tmp)-1;
		bool needadd = true; //###要改
		if (g.sec<=currenttime) //以目前時間為準
		{
			if (eph[sv].toc.sec<g.sec) //取最接近的
			{
				needadd = true;
			}
		}
...
void usage(void)
{
	printf("Usage: hackrf_gpsspoof [options]\n"
		"Options:\n"
		"  -e <gps_nav>     RINEX navigation file for GPS ephemerides (required)\n"
		"  -l <location>    Lat,Lon,Hgt (static mode) e.g. 30.286502,120.032669,100\n");
	return;
}
#define MAXBUFFERSIZE   (260000*2*2) //0.5second
#define PERMOTIONIQSIZE (260000*2) //0.1second
CRITICAL_SECTION g_cs;
size_t g_endoffset = 0;
size_t g_totalsent = 0;
char *transbuffer = (char*)calloc(MAXBUFFERSIZE,1);
double xyz[3];
int tx_callback(hackrf_transfer* transfer) 
{
	size_t bytes_to_send;
	bytes_to_send = transfer->valid_length;
	while (g_endoffset < PERMOTIONIQSIZE)
	{
		Sleep(1);
	}
	EnterCriticalSection(&g_cs);
	memcpy(transfer->buffer,&transbuffer[0],bytes_to_send);
	memcpy(&transbuffer[0],&transbuffer[0]+bytes_to_send,MAXBUFFERSIZE-bytes_to_send);
	g_endoffset -= bytes_to_send;
	g_totalsent+=bytes_to_send;
	double llh[3];
	xyz2llh(xyz,llh);
	printf("\rTime:%lf\tLatitude:%lf\tLongitude:%lf",(g_totalsent/PERMOTIONIQSIZE/10.0),llh[0]*R2D,llh[1]*R2D);
...
int main(int argc, char *argv[])
{
	...
	printf("call hackrf_set_freq(%I64d Hz/%.03f MHz)\n",
	...
	while ((result=getopt(argc,argv,"e:l:"))!=-1)
	{
		switch (result)
		{
		case 'e':
	...
	struct tm * now ;
	now = gmtime(&t);
	double csec = (double)(now->tm_wday*SECONDS_IN_DAY + now->tm_hour*SECONDS_IN_HOUR 
		+ now->tm_min *SECONDS_IN_MINUTE + now->tm_sec);
	neph = readRinexNav(eph,csec, navfile);
	if (neph==-1)
	{
	...
	*/
	...
	for (sv=0; sv<MAX_SAT; sv++)
	{
		if (eph[sv].vflg==1)
		{
			satpos(eph[sv], g0, pos, vel, clk); //計算某時間衛星的位置
			subVect(los, pos, xyz); //計算位置向量
			ecef2neu(los, tmat, neu); //轉換座標格式
			neu2azel(azel, neu); //計算方位角,仰角

			if (azel[1]>elvmask) //只處理仰角>0的衛星 //####
			if (nsat<MAX_CHAN)
			{
				chan[nsat].prn = sv+1;
				nsat++;
				printf("Satellite:%02d Azimuth:%6.1f Elevation:%5.1f\n", sv+1, azel[0]*R2D, azel[1]*R2D);
			}
			else
			if (nsat>=MAX_CHAN)
			break;
	...
	for (i=0; i<nsat; i++)
	{
		...
		computeRange(&tmp, eph[sv], grx, xyz);
		r_xyz = tmp.range;
	...
	while (true)
	{
		...
		while (g_endoffset+PERMOTIONIQSIZE>MAXBUFFERSIZE)
		{
			Sleep(1);
		}
		EnterCriticalSection(&g_cs);
		memcpy(&transbuffer[g_endoffset],iq_buff,PERMOTIONIQSIZE);
		g_endoffset+=PERMOTIONIQSIZE;
		LeaveCriticalSection(&g_cs);
		grx.sec += 0.1;
		#define distance 0.0000001
		//#define distance 0.00001
		JOYINFO joyinfo; 
		if (joyGetNumDevs()>0 && joyGetPos(JOYSTICKID1, &joyinfo) == JOYERR_NOERROR )
		{
			...
			{
				int x = 100.0*((joyinfo.wXpos-32767.0)/32767.0);
				int y = 100.0*((joyinfo.wYpos-32767.0)/32767.0);
				y = ~y+1;
				double deltax = distance;
				double deltay = distance;
				deltax = deltax *(x/100.0);
				deltay = deltay *(y/100.0);
				if (x>10 || x<-10)
				llh[1] +=deltax;
				if (y>10 || y<-10)
				llh[0] +=deltay;
			}
			//printf("%lf %lf %lf %lf\n",llh[1],llh[0],deltax,deltay);
			//printf("%d %d %d\n",x,y,joyinfo.wButtons);
		}
		else
		if (_kbhit())
		{
